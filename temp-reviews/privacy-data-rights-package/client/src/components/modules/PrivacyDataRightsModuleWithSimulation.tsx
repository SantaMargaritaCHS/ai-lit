...
// Developer mode components will be passed as props instead of using hook directly
...
export function PrivacyDataRightsModule({ onComplete, isDevMode = false, showDevPanel = false }: PrivacyDataRightsModuleProps) {
  ...
  // Enhanced AI response generation with error handling (moved to component level)
  const generateRealAIResponse = async (prompt: string, includeData: boolean = false) => {
    try {
      ...
      return data.response || "I can help you with that. Let me provide some suggestions...";
    } catch (error) {
  ...
  const getAIFeedback = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/gemini/feedback', {
        ...
        body: JSON.stringify({
          prompt: `Teacher reflection on AI privacy training:
            Biggest takeaway: ${sanitizedTakeaway}
            Protection steps: ${sanitizedProtection}
            Student education approach: ${sanitizedEducation}`,
          context: 'Provide encouraging feedback on their understanding of AI privacy and data protection. Acknowledge their specific insights and reinforce the importance of both protecting student data and teaching students about AI privacy risks.'
        }),
      });
      ...
      if (isMountedRef.current) {
        setAiFeedback(data.feedback || 'Great reflection! Your commitment to protecting student privacy is commendable.');
      }
    } catch (error) {
